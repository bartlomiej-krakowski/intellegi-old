...
Element.prototype.removeEvent = function (name) {
...
class Header {
  ...
  setVars() {
    ...
    this.searchWrapper  = document.querySelector('.header__search')
    this.searchToggle   = this.element.querySelector('.header__searchFormSubmit')
    this.notifyWrapper  = document.querySelector('.header__notify')
    this.notifyToggle   = this.element.querySelector('.header__notifyToggle')
    this.profileWrapper = document.querySelector('.header__profile')
    this.profileToggle  = this.element.querySelector('.header__profileToggle')
    this.classes        = {
  ...
  setEvents() {
    ...
    this.searchWrapper.addEventListener('click', (e) => { e.stopPropagation() })
    this.notifyWrapper.addEventListener('click', (e) => { e.stopPropagation() })

    this.searchToggle.addEventListener('click',  (e) => {
      this.toggleContent(e, this.searchWrapper, this.classes.search, true)
    })
    this.notifyToggle.addEventListener('click',  (e) => {
      this.toggleContent(e, this.notifyWrapper, this.classes.notify)
    })
    this.profileToggle.addEventListener('click',  (e) => {
      this.toggleContent(e, this.profileWrapper, this.classes.profile)
    })
  ...
  toggleContent(e, wrapper, wrapperClass, lockClose = false) {

    const isClass = wrapper.hasClass(wrapperClass)
    e.stopPropagation()

    if (lockClose && isClass)
      return
  ...
  closeAll() {
    ...
    this.searchWrapper.removeClass(this.classes.search)
    this.notifyWrapper.removeClass(this.classes.notify)
    this.profileWrapper.removeClass(this.classes.profile)
...
class CommentsSlider {
  ...
  setVars() {
    ...
    this.section = document.querySelector('.commentsSlider')

    if (!this.section)
      return

    this.wrapper = this.section.querySelector('.commentsSlider__wrapper')
    this.navPrev = this.wrapper.querySelector('.commentsSlider__navPrev')
    this.navNext = this.wrapper.querySelector('.commentsSlider__navNext')
...
class LatestContent {
  ...
  setVars() {
    ...
    this.section = document.querySelector('.latestContent')

    if (!this.section)
      return

    this.buttons = this.section.querySelectorAll('.latestContent__button')
    this.tabs    = this.section.querySelectorAll('.latestContent__tab')
    this.classes = {
      button : 'latestContent__button--active',
      tab    : 'latestContent__tab--active'
    }
  ...
  setTab(index) {
    ...
    this.buttons.removeClass(this.classes.button)
    this.tabs.removeClass(this.classes.tab)

    this.buttons[index].addClass(this.classes.button)
    this.tabs[index].addClass(this.classes.tab)
...
class PostsSlider {
  ...
  setVars() {
    ...
    this.section = document.querySelector('.postsSlider')

    if (!this.section)
      return

    this.images   = this.section.querySelectorAll('.postsSlider__image')
    this.contents = this.section.querySelectorAll('.postsSlider__content')
    this.number   = this.section.querySelector('.postsSlider__navCountCurrent')
    this.prev     = this.section.querySelector('.postsSlider__prev')
    this.next     = this.section.querySelector('.postsSlider__next')
    this.classes  = {
      image   : 'postsSlider__image--active',
      content : 'postsSlider__content--active'
    }
    this.current  = 0
    this.limit    = this.contents.length - 1
  ...
  setSlide() {
    ...
    const value = this.isArabic ? arabicNumbers((this.current + 1)) : (this.current + 1)
    this.number.innerHTML = value

    this.images.removeClass(this.classes.image)
    this.contents.removeClass(this.classes.content)

    this.images[this.current].addClass(this.classes.image)
    this.contents[this.current].addClass(this.classes.content)
...
class ProfileHeader {
  ...
  setVars() {
    ...
    this.section = document.querySelector('.profileHeader')

    if (!this.section)
      return

    this.select = this.section.querySelector('.profileHeader__weatherCitySelect')
    this.label  = this.select.getAttribute('data-label')
    this.types  = this.section.querySelectorAll('.profileHeader__weatherType')
  ...
  initSelectr() {
    ...
    this.selectr = new Selectr(this.select, {
      searchable  : false,
      customClass : 'selectGreen'
    })

    this.selectr.on('selectr.init', () => {

      const label = this.select.parentNode.querySelector('.selectr-label')
      label.innerHTML = this.label.replace('%s', label.innerHTML)

    })

    this.selectr.on('selectr.change', (option) => {

      const label = this.select.parentNode.querySelector('.selectr-label')
      const value = option.value
      label.innerHTML = this.label.replace('%s', option.innerHTML)

      this.types.forEach((element) => {

        if (element.getAttribute('data-type') == value)
          element.addClass('profileHeader__weatherType--active')
        else
          element.removeClass('profileHeader__weatherType--active')
...
class TabsSlider {
  ...
  setVars() {
    ...
    this.section = document.querySelector('.tabsSlider')

    if (!this.section)
      return

    this.wrapper = this.section.querySelector('.tabsSlider__wrapperInner')
    this.navPrev = this.wrapper.querySelector('.tabsSlider__navPrev')
    this.navNext = this.wrapper.querySelector('.tabsSlider__navNext')
...
class Widgets {
  ...
  initMasonry() {
    ...
    this.isotope = new Isotope('.widgets__items', {
      isOriginLeft: false,
      itemSelector: '.widgets__item',
      masonry: {
        columnWidth: '.widgets__sizer'
      }
    });
